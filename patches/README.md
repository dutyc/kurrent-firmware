...
| `0012-nvmetcp-nbct-acpi-table.patch` | `src/drivers/block/nbct.c` (new), `src/include/ipxe/nbct.h` (new), `src/include/ipxe/errfile.h`, `src/include/ipxe/nvmetcp.h`, `src/net/tcp/nvmetcp.c` | NVMe boot credentials table (NBCT): a custom ACPI table installed at sanboot time (iBFT-style `acpi_model` registration) carrying one fixed-size record per authenticated NVMe/TCP session (transport address, subsystem NQN, host NQN, DH-HMAC-CHAP secret); consumed by the initramfs `nbft-connect` (see `../initramfs-nbft/`) to reconnect with `--dhchap-secret`/`--hostnqn` without a second network fetch of credentials |
...
- the new NVMe/TCP driver files in `0006`/`0007` (`nvmetcp.c`, `nvmetcp_auth.c`, `nvmetcp.h`, `nvmetcp_test.c`) are original kurrent-firmware implementations carrying `FILE_LICENCE ( GPL2_ONLY )`; they are **GPL-2.0 only** and may not be redistributed under UBDL;
- the device-identity files in `0008`/`0009`/`0010` (`efi_nvs.c`, `tofu.c`, `tofu.h`, `devicekey_cmd.c`) are original kurrent-firmware implementations carrying `FILE_LICENCE ( GPL2_OR_LATER_OR_UBDL )`, matching upstream iPXE's dual licence;
- the NBCT files in `0012` (`nbct.c`, `nbct.h`) are original kurrent-firmware implementations carrying `FILE_LICENCE ( BSD2 )` (kernel-style licence, matching the ACPI-table precedent in upstream iPXE's `ibft.c`);
